...
// GetMiniTokenOrders returns order information based up on order id
func GetMiniTokenOrders(rw http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	id := vars["id"]
	...
	order, err := s.client.GetMiniTokenOrder(id)
	if err != nil {
		s.l.Printf("failed to request order information: %s\n", err)
		return
	}
